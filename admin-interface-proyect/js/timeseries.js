...
(function() {
    ...
    var timeseries = function(spaced, data, enableBrush, xFormat, css) {
        classd = spaced.replace(new RegExp(" "), ".");
        render(classd, spaced, data, enableBrush, xFormat,css);
    }
    ...
    */
	...
	//css format added by romina follow structure: css.width, css.height, css.margin { top: ,right: ,bottom: ,left: 
    function render(classd, spaced, data, enableBrush, x_Format, css) {
		...
		if(css != null && css.margin !=null) margin = css.margin;
       
        var width = css!=null? css.width: window.innerWidth - 150;
        var height = (lessThanDay(padding.pad)) ? (100 - margin.top - margin.bottom) : ((css!=null? css.height:300) - margin.top - margin.bottom);
        ...
        var xFormat , yFormat;
			...
			if (lessThanDay(padding.pad)) {
				xFormat = (typeof x_Format !== 'string') ?"%H:%M":  x_Format;
				yFormat = "%m/%d/%y";
				y.domain(d3.extent([padding.minDate]));
			} else {
				xFormat = (typeof x_Format !== 'string') ? "%m/%d/%y" :  x_Format;
				yFormat = "%H:%M";
